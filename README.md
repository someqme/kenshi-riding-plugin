...
- 物种名与游戏内 `getName()` 的字节一致（GBK/GB18030 编码，请勿用会改变编码的编辑器改动物种名）。

## ⚠️ 已知限制 / Known limitations

- **巨型坐骑**（乌龟、利维坦）：骑手的名字标签与镜头焦点会落到地面高度 —— 引擎对"被携带角色"的 UI 锚点做了地形 fallback，属**引擎级限制**，DLL 无法修正。
- 部分物种（如喙嘴兽）骑手可能不易用鼠标点中。
- 铁蜘蛛等物种坐姿需用热键**现场标定**。
- 骑手被原生携带时无法主动攻击（引擎限制）；由坐骑代为战斗。
